...
export let verify2faTrend = new Trend('verify2fa_duration_ms');
...
// Pure JS TOTP (RFC 6238, 6 digits, 30s, Base32)
...
export default function () {
    ...
    let payload = JSON.stringify({ token: tempToken, code: code });
    let res = http.post(`${BASE_URL}/auth/2fa/verify`, payload, { headers: { 'Content-Type': 'application/json' } });
    verify2faTrend.add(res.timings.duration);
    check(res, { '2fa verify status 200': (r) => r.status === 200 });
    sleep(1);
} 